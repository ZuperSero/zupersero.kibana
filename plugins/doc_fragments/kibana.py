...
options:
  ...
  urls:
    description:
      ...
      - Can also be set with the C(KIBANA_URLS) environment variable.
  ...
  username:
    description:
      ...
      - Can also be set with the C(KIBANA_USERNAME) environment variable.
    type: str
  password:
    description:
      - Password for basic authentication.
      - Can also be set with the C(KIBANA_PASSWORD) environment variable.
    type: str
  api_key:
    description:
      - Encoded API key for C(ApiKey) authentication.
      - Can also be set with the C(KIBANA_API_KEY) environment variable.
    type: str
  bearer_token:
    description:
      - Token for C(Bearer) authentication.
      - Can also be set with the C(KIBANA_BEARER_TOKEN) environment variable.
    type: str
  headers:
    description:
      - Additional HTTP headers.
      - Header values are treated as sensitive and are redacted from failures.
      - Can also be set as a JSON object with the C(KIBANA_HEADERS) environment variable.
    type: dict
    default: {}
  space:
    description:
      - Kibana space used by modules that support space-scoped APIs.
      - Can also be set with the C(KIBANA_SPACE) environment variable.
    type: str
    default: default
  validate_certs:
    description:
      - Whether to validate TLS certificates.
      - Can also be set with the C(KIBANA_VALIDATE_CERTS) environment variable.
    type: bool
    default: true
  ca_path:
    description:
      - Path to a PEM CA certificate bundle.
      - Can also be set with the C(KIBANA_CA_PATH) environment variable.
    type: path
  ca_data:
    description:
      - PEM CA certificate data.
      - Can also be set with the C(KIBANA_CA_DATA) environment variable.
    type: str
  client_cert:
    description:
      - Path to a PEM client certificate chain.
      - Can also be set with the C(KIBANA_CLIENT_CERT) environment variable.
    type: path
  client_key:
    description:
      - Path to the private key for I(client_cert).
      - Can also be set with the C(KIBANA_CLIENT_KEY) environment variable.
    type: path
  certificate_fingerprint:
    description:
      - SHA-256 fingerprint of the expected TLS peer certificate.
      - Can also be set with the C(KIBANA_CERTIFICATE_FINGERPRINT) environment variable.
  ...
  timeout:
    description:
      ...
      - Can also be set with the C(KIBANA_TIMEOUT) environment variable.
    type: int
    default: 30
  retries:
    description:
      - Number of retries after the initial request for transport errors, C(408), C(429), and C(5xx) responses.
      - Can also be set with the C(KIBANA_RETRIES) environment variable.
    type: int
    default: 3
  retry_pause:
    description:
      - Base delay in seconds for jittered exponential retry backoff.
      - Can also be set with the C(KIBANA_RETRY_PAUSE) environment variable.
    type: float
    default: 1.0
  retry_status_codes:
    description:
  ...
  retry_mutating_requests:
    description:
      ...
      - Can also be set with the C(KIBANA_RETRY_MUTATING_REQUESTS) environment variable.
    type: bool
    default: false
  force_basic_auth:
    description:
